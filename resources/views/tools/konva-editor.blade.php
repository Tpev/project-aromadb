...
<x-app-layout>
    ...
    <script>
    // Inline Konva editor logic (no Vite)
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        if (btnUndo) {
            btnUndo.addEventListener('click', () => {
                ...
                // Simple: reload – you can later implement restore from JSON if needed
        ...
        // === TEMPLATE REGISTRY (dynamic) ============================
        const templateHandlers = {};

        function registerTemplate(id, handler) {
            templateHandlers[id] = handler;
        }
    ...
    mainLayer.add(card);
    registerElement(card, 'shape-rect', 'Carte citation');

    // Small brand tag at top
    const tag = new Konva.Rect({
        x: stage.width() * 0.11,
        y: stage.height() * 0.16,
        width: 140,
        height: 26,
    ...
    mainLayer.add(tag);
    registerElement(tag, 'shape-rect', 'Tag');

    const tagText = new Konva.Text({
        text: 'CITATION BIEN-ÊTRE',
        x: stage.width() * 0.11,
        y: stage.height() * 0.16 + 4,
        width: 140,
    ...
    const tagGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    tagGroup.add(tagText);
    mainLayer.add(tagGroup);
    registerElement(tagGroup, 'text', 'Tag texte');

    // Opening quote mark
    const bigQuote = new Konva.Text({
        text: '“',
        x: stage.width() * 0.14,
        y: stage.height() * 0.23,
        width: 40,
    ...
    mainLayer.add(bigQuote);
    registerElement(bigQuote, 'text', 'Guillemet');

    // Main quote text
    const quoteText = new Konva.Text({
        text: 'Votre bien-être est un chemin que vous empruntez chaque jour, pas une destination à atteindre en une fois.',
        x: stage.width() * 0.20,
        y: stage.height() * 0.24,
    ...
    const brandText = new Konva.Text({
        text: 'www.votresite.fr • @votrecompte',
        x: stage.width() * 0.20,
        y: stage.height() * 0.63,
        width: stage.width() * 0.6,
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
    ...
    const brandGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    brandGroup.add(brandText);
    mainLayer.add(brandGroup);
    registerElement(brandGroup, 'text', 'Signature');
...
function addPromoTemplate() {
    ...
    mainLayer.add(card);
    registerElement(card, 'shape-rect', 'Carte promo');
    ...
    registerElement(topBar, 'shape-rect', 'Bande haute');

    const topBarText = new Konva.Text({
    ...
    const topBarGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    topBarGroup.add(topBarText);
    mainLayer.add(topBarGroup);
    registerElement(topBarGroup, 'text', 'Titre promo');
    ...
    const subtitleGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    subtitleGroup.add(subtitle);
    mainLayer.add(subtitleGroup);
    registerElement(subtitleGroup, 'text', 'Sous-titre promo');

    const body = new Konva.Text({
        text: 'Une séance pour faire le point, comprendre vos besoins et repartir avec un plan d’action concret.',
        x: card.x() + 32,
        y: card.y() + 120,
        width: card.width() * 0.55,
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
    ...
    registerElement(bodyGroup, 'text', 'Texte promo');

    // Price area
    const priceBadge = new Konva.Circle({
        x: card.x() + card.width() - 100,
        y: card.y() + 130,
        radius: 70,
        fill: '#f97316',
    ...
    mainLayer.add(priceBadge);
    registerElement(priceBadge, 'shape-circle', 'Badge prix');
    ...
    const priceSub = new Konva.Text({
        text: 'sur votre 1ʳᵉ séance',
        x: priceBadge.x() - 60,
        y: priceBadge.y() + 10,
        width: 120,
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
    ...
    const priceGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    priceGroup.add(priceText);
    priceGroup.add(priceSub);
    mainLayer.add(priceGroup);
    registerElement(priceGroup, 'text', 'Texte badge prix');

    // Image placeholder on right
    const imagePlaceholder = new Konva.Rect({
        x: card.x() + card.width() * 0.56,
        y: card.y() + 200,
        width: card.width() * 0.34,
    ...
    mainLayer.add(imagePlaceholder);
    registerElement(imagePlaceholder, 'shape-rect', 'Zone visuel');

    const imageHint = new Konva.Text({
        text: 'Ajoutez votre photo ou\nune image de séance ici',
        x: imagePlaceholder.x(),
        y: imagePlaceholder.y() + imagePlaceholder.height() / 2 - 24,
    ...
    const imageGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    imageGroup.add(imageHint);
    mainLayer.add(imageGroup);
    registerElement(imageGroup, 'text', 'Texte visuel');

    // CTA bar at bottom
    const ctaRect = new Konva.Rect({
        x: card.x() + 32,
        y: card.y() + card.height() - 70,
        width: card.width() - 64,
        height: 40,
        cornerRadius: 999,
        fill: '#0f172a'
    ...
    const ctaText = new Konva.Text({
        text: 'Réserver votre séance • www.votresite.fr',
        x: ctaRect.x(),
        y: ctaRect.y() + 10,
        width: ctaRect.width(),
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
...
function addEventTemplate(eventData = null) {
    ...
    mainLayer.add(card);
    registerElement(card, 'shape-rect', 'Carte atelier');
    ...
    registerElement(dateGroup, 'text', 'Texte date'); // keep this name

    // Main title
    const titleText = new Konva.Text({
        text: (eventData && eventData.title) || 'Atelier : Découvrir les huiles essentielles',
        x: dateRect.x() + dateRect.width() + 24,
        y: card.y() + 46,
        width: card.width() - dateRect.width() - 60,
        fontFamily: '"Playfair Display", "Times New Roman", serif',
        fontSize: 26,
        fill: '#0f172a',
        align: 'left',
    ...
    registerElement(titleGroup, 'text', 'Titre atelier'); // used by applyEventToEventTemplate
    ...
    const infoGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    infoGroup.add(infoText);
    mainLayer.add(infoGroup);
    registerElement(infoGroup, 'text', 'Infos atelier'); // used by applyEventToEventTemplate
    ...
    const divider = new Konva.Line({
        points: [
            card.x() + 28,
            card.y() + 190,
            card.x() + card.width() - 28,
            card.y() + 190
        ],
        stroke: '#e5e7eb',
        strokeWidth: 1
    });
    mainLayer.add(divider);
    registerElement(divider, 'shape-rect', 'Séparateur');

    // Description / program block
    const programText = new Konva.Text({
        text: 'Au programme :\n• Comprendre les bases de l’aromathérapie\n• Manipuler quelques huiles essentielles clés\n• Répondre à vos questions en direct',
        x: card.x() + 32,
        y: card.y() + 206,
        width: card.width() - 64,
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
    ...
    const programGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    programGroup.add(programText);
    mainLayer.add(programGroup);
    registerElement(programGroup, 'text', 'Programme');
    ...
    const urlText = new Konva.Text({
        text: (eventData && eventData.url) || 'Réservation sur : www.votresite.fr/atelier',
        x: bottomRect.x(),
        y: bottomRect.y() + 18,
    ...
    registerElement(urlGroup, 'text', 'URL'); // used by applyEventToEventTemplate
  ...
  function addTestimonialTemplate() {
    ...
    const labelGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    labelGroup.add(label);
    mainLayer.add(labelGroup);
    registerElement(labelGroup, 'text', 'Label avis');
    ...
    const avatarGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    avatarGroup.add(initials);
    mainLayer.add(avatarGroup);
    registerElement(avatarGroup, 'text', 'Initiales');
    ...
    const starsGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    starsGroup.add(stars);
    mainLayer.add(starsGroup);
    registerElement(starsGroup, 'text', 'Étoiles');

    // Body text
    const bodyText = new Konva.Text({
        text: '« J’ai retrouvé de l’énergie et des habitudes plus saines au quotidien. ' +
              'L’accompagnement m’a permis de passer à l’action en douceur, à mon rythme, ' +
              'avec un suivi bienveillant et structuré. »',
        x: card.x() + 28,
        y: card.y() + 170,
        width: card.width() - 56,
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
    ...
    const footer = new Konva.Text({
        text: '@votrecompte • www.votresite.fr',
        x: card.x() + 28,
    ...
    const footerGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    footerGroup.add(footer);
    mainLayer.add(footerGroup);
    registerElement(footerGroup, 'text', 'Footer avis');
...
function addChecklistTemplate() {
    ...
    labelGroup.add(label);
    mainLayer.add(labelGroup);
    registerElement(labelGroup, 'text', 'Label checklist');
    ...
    const items = [
        ...
        'Couper les écrans 30 minutes avant',
    ...
    const note = new Konva.Text({
        text: 'Astuce : imprimez cette checklist ou gardez-la en modèle pour vos stories.',
        x: card.x() + 28,
        y: card.y() + card.height() - 60,
        width: card.width() - 56,
        fontFamily: '"Raleway", system-ui, -apple-system, sans-serif',
    ...
    const noteGroup = new Konva.Group({ x: 0, y: 0, draggable: true });
    noteGroup.add(note);
    mainLayer.add(noteGroup);
    registerElement(noteGroup, 'text', 'Note checklist');

    mainLayer.draw();
}

        // Register templates with IDs matching config/konva.php
        registerTemplate('quote',        addQuoteTemplate);
        registerTemplate('promo',        addPromoTemplate);
        registerTemplate('event',        () => addEventTemplate());
        registerTemplate('testimonial',  addTestimonialTemplate);
        registerTemplate('tip',          addTipTemplate);
        registerTemplate('before_after', addBeforeAfterTemplate);
        registerTemplate('checklist',    addChecklistTemplate);

        // Bind template buttons (dynamic from config via left-sidebar)
        document.querySelectorAll('.js-template-btn').forEach(btn => {
            btn.addEventListener('click', () => {
                const id = btn.dataset.template;
                const handler = templateHandlers[id];
                if (typeof handler === 'function') {
                    handler();
                } else {
                    console.warn('No handler registered for template:', id);
                }
            });
        });
        ...
        // === Event selector -> apply to event template ===
        if (eventSelector) {
