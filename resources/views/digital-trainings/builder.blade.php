...
<x-app-layout>
    ...
    <div class="container mt-6">
        <div class="mx-auto max-w-6xl space-y-4">
            @if(session('success'))
                <div class="rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-2 text-sm text-emerald-800 flex items-center justify-between gap-3">
                ...
                <div class="w-full md:w-1/3 space-y-4">
                    ...
                    <div class="bg-white shadow-sm rounded-2xl p-4 border border-slate-100">
                        ...
                        <p class="text-[11px] text-slate-500 mb-3">
                            {{ __('Découpez votre formation en chapitres (introduction, modules thématiques, conclusion, etc.).') }}
                        </p>
                    ...
                    <div class="text-[11px] text-slate-500">
                        {{ __('Astuce : cliquez sur “Texte”, “Vidéo” ou “PDF” dans un module pour ouvrir un grand éditeur à droite. Utilisez le bouton “⛶” pour passer en plein écran.') }}
                    </div>
                </div>

                {{-- RIGHT AREA: Modules list (left) + big editor (right) --}}
                <div class="w-full md:w-2/3 space-y-4">
                    <div class="flex flex-col lg:flex-row gap-4">
                        {{-- CURRICULUM LIST (LEFT SIDE) --}}
                        <div class="w-full lg:w-1/2 bg-white shadow-sm rounded-2xl p-4 border border-slate-100">
                            <div class="flex items-center justify-between mb-3">
                                <div>
                                    ...
                                    <p class="text-[11px] text-slate-500">
                                        {{ __('Cliquez sur un contenu pour l’éditer dans le grand éditeur à droite.') }}
                                    </p>
                                </div>
                            </div>
                                                        ...
                                                        <div class="flex-1">
                                                            ...
                                                            @if($block->type === 'text' && $block->content)
                                                                <div class="prose prose-xs max-w-none text-slate-700 line-clamp-3">
                                                                    {!! Str::limit(strip_tags($block->content), 120) !!}
                                                                </div>
                        ...
                        {{-- BIG EDITOR PANEL (RIGHT SIDE) + FULLSCREEN --}}
                        <div
                            id="editor-panel-wrapper"
                            :class="maximizedEditor
                                ? 'fixed inset-0 z-40 flex items-center justify-center p-4 bg-slate-900/60'
                                : 'w-full lg:w-1/2'"
                        >
                            <div
                                id="editor-panel"
                                :class="maximizedEditor
                                    ? 'bg-white shadow-2xl rounded-2xl border border-slate-200 w-full max-w-5xl h-full lg:h-[90vh] flex flex-col'
                                    : 'bg-white shadow-sm rounded-2xl border border-slate-100 flex flex-col min-h-[420px] p-4'"
                                class="p-4"
                            >
                                <div class="flex items-center justify-between mb-3 border-b border-slate-100 pb-2">
                                    <div>
                                        ...
                                        <p class="text-[11px] text-slate-500" x-show="!activeAction">
                                            {{ __('Sélectionnez un contenu ou créez-en un depuis la colonne de gauche.') }}
                                        </p>
                                        <p class="text-[11px] text-slate-500" x-show="activeAction">
                                            {{ __('Vous éditez le contenu sélectionné. Les changements sont enregistrés lors du clic sur “Enregistrer” ou “Créer”.') }}
                                        </p>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <button type="button"
                                                @click="maximizedEditor = !maximizedEditor; scrollToEditor();"
                                                class="inline-flex items-center rounded-full border border-slate-200 px-3 py-1 text-[11px] font-semibold text-slate-700 hover:bg-slate-50 bg-slate-50/60">
                                            <span x-show="!maximizedEditor">⛶ {{ __('Agrandir') }}</span>
                                            <span x-show="maximizedEditor">↙ {{ __('Réduire') }}</span>
                                        </button>
                                    </div>
                                </div>
                                ...
                                <div class="flex-1 overflow-y-auto px-1 pb-2">
                                    ...
                                    <div x-show="!activeAction" class="h-full flex items-center justify-center text-center text-[12px] text-slate-400">
                                        <div class="max-w-sm">
                                            <p>{{ __('Choisissez un module à gauche, puis cliquez sur “Texte”, “Vidéo” ou “PDF” pour créer un contenu, ou sur “Éditer” pour modifier un contenu existant.') }}</p>
                                            <p class="mt-2">{{ __('Utilisez le bouton “⛶” pour passer en mode plein écran et écrire confortablement.') }}</p>
                                        </div>
                                    </div>
                                    ...
                                    @foreach($training->modules as $module)
                                        @foreach($module->blocks as $block)
                                            <div x-show="activeAction === 'edit' && activeModuleId === {{ $module->id }} && activeBlockId === {{ $block->id }}"
                                                 x-cloak
                                                 class="space-y-3">
                                                ...
                                                <form action="{{ route('digital-trainings.blocks.update', [$training, $module, $block]) }}"
                                                    ...
                                                    <div class="flex justify-between items-center pt-3 border-t border-slate-100">
                                                        <button type="button"
                                                                @click="activeAction=null; activeBlockId=null; activeModuleId=null; activeType=null;"
                                                                class="rounded-md border border-slate-200 px-3 py-1.5 text-[11px] text-slate-600 hover:bg-slate-50">
                                                            {{ __('Fermer sans quitter l’éditeur') }}
                                                        </button>
                                                        <div class="flex gap-2">
                                                            <button type="button"
                                                                    @click="activeAction=null; activeBlockId=null; activeModuleId=null; activeType=null; maximizedEditor=false;"
                                                                    class="rounded-md border border-slate-200 px-3 py-1.5 text-[11px] text-slate-600 hover:bg-slate-50">
                                                                {{ __('Annuler') }}
                                                            </button>
                                                            <button type="submit"
                                    ...
                                    @foreach($training->modules as $module)
                                        ...
                                        <div x-show="activeAction === 'create' && activeModuleId === {{ $module->id }} && activeType === 'text'"
                                            ...
                                            <form action="{{ route('digital-trainings.blocks.store', [$training, $module]) }}"
                                                ...
                                                <div class="flex justify-between items-center pt-3 border-t border-slate-100">
                                                    <button type="button"
                                                            @click="activeAction=null; activeModuleId=null; activeType=null;"
                                                            class="rounded-md border border-slate-200 px-3 py-1.5 text-[11px] text-slate-600 hover:bg-slate-50">
                                                        {{ __('Fermer sans quitter l’éditeur') }}
                                                    </button>
                                                    <div class="flex gap-2">
                                                        <button type="button"
                                                                @click="activeAction=null; activeModuleId=null; activeType=null; maximizedEditor=false;"
                                                                class="rounded-md border border-slate-200 px-3 py-1.5 text-[11px] text-slate-600 hover:bg-slate-50">
                                                            {{ __('Annuler') }}
                                                        </button>
                                                        <button type="submit"
                    ...
                    <div class="flex justify-between items-center text-[11px] text-slate-500 pt-1">
                        ...
                        <div class="flex items-center gap-2">
                            <span>{{ __('Les modifications sont enregistrées à chaque clic sur “Enregistrer” ou “Créer”.') }}</span>
                            <a href="{{ route('digital-trainings.preview', $training) }}"
    ...
    <script>
        function scrollToEditor() {
            const el = document.getElementById('editor-panel-wrapper');
            if (el) {
