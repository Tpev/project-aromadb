...
@php
    ...
    $freeAccessIsOpen = (bool) old('free_access_is_open', $training->free_access_is_open ?? false);
...
<x-app-layout>
    ...
    <div class="container mt-6" x-data="{ isFree: {{ old('is_free', $isFree) ? 'true' : 'false' }}, freeAccessIsOpen: {{ $freeAccessIsOpen ? 'true' : 'false' }} }">
                ...
                <div class="rounded-2xl border border-slate-100 bg-slate-50 p-4">
                    <div class="flex items-start justify-between gap-3">
                        <div>
                            ...
                            <p class="text-[11px] text-slate-500">
                                {{ __('Optionnel : afficher une case à cocher au checkout avec le document global défini dans Informations de l’entreprise.') }}
                            </p>
                        </div>
