<x-app-layout>
    ...
    <div class="py-6" x-data="{
        ...
        estimate: null,
        estimating: false,
        word(count, singular, plural) { return Number(count) === 1 ? singular : plural },
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                    <div>
                                        ...
                                        @if($campaign->status === 'sent')
                                            <p class="mt-2 text-xs text-gray-600">{{ $campaign->sent_count }} {{ (int) $campaign->sent_count === 1 ? 'envoyé' : 'envoyés' }} · {{ $campaign->skipped_count }} {{ (int) $campaign->skipped_count === 1 ? 'exclu' : 'exclus' }} · {{ $campaign->eligible_count }} {{ (int) $campaign->eligible_count === 1 ? 'éligible' : 'éligibles' }}</p>
                                            <p class="mt-1 text-xs text-gray-500">{{ data_get($campaign->summary_json, 'no_consent', 0) }} sans consentement · {{ data_get($campaign->summary_json, 'unsubscribed', 0) }} {{ (int) data_get($campaign->summary_json, 'unsubscribed', 0) === 1 ? 'désinscription' : 'désinscriptions' }} · {{ data_get($campaign->summary_json, 'suppressed', 0) }} {{ (int) data_get($campaign->summary_json, 'suppressed', 0) === 1 ? 'suppression' : 'suppressions' }}</p>
                                            <p class="mt-1 text-xs font-medium {{ ($campaign->failed_deliveries_count + $campaign->bounced_deliveries_count + $campaign->complained_deliveries_count) > 0 ? 'text-amber-800' : 'text-gray-500' }}">{{ $campaign->failed_deliveries_count }} {{ (int) $campaign->failed_deliveries_count === 1 ? 'échec' : 'échecs' }} · {{ $campaign->bounced_deliveries_count }} {{ (int) $campaign->bounced_deliveries_count === 1 ? 'rejet' : 'rejets' }} · {{ $campaign->complained_deliveries_count }} {{ (int) $campaign->complained_deliveries_count === 1 ? 'plainte' : 'plaintes' }}</p>
                                        @endif
                                    </div>
                ...
                <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                    ...
                    <form x-ref="campaignForm" @change.debounce.250ms="refreshEstimate()" method="POST" action="{{ $editCampaign ? route('offer-journeys.message-campaigns.update', $editCampaign) : route('offer-journeys.message-campaigns.store') }}" class="mt-4 space-y-4">
                        ...
                        <div class="rounded-md border border-[#dfe6c7] bg-[#f7f9ec] p-3 text-xs text-gray-700" aria-live="polite">
                            ...
                            <div x-show="!estimating && summary"><p><strong x-text="summary?.matching || 0"></strong> <span x-text="word(summary?.matching, 'personne correspond', 'personnes correspondent')"></span> à l’audience.</p><p class="mt-1 text-[#526509]"><strong x-text="summary?.eligible || 0"></strong> <span x-text="word(summary?.eligible, 'destinataire peut', 'destinataires peuvent')"></span> actuellement recevoir ce message.</p><p class="mt-1 text-gray-500"><span x-text="summary?.no_consent || 0"></span> sans consentement · <span x-text="summary?.unsubscribed || 0"></span> <span x-text="word(summary?.unsubscribed, 'désinscription', 'désinscriptions')"></span> · <span x-text="summary?.suppressed || 0"></span> <span x-text="word(summary?.suppressed, 'suppression', 'suppressions')"></span>, dont <span x-text="summary?.bounce_or_complaint || 0"></span> <span x-text="word(summary?.bounce_or_complaint, 'rejet ou plainte', 'rejets ou plaintes')"></span> · <span x-text="summary?.frequency_limited || 0"></span> <span x-text="word(summary?.frequency_limited, 'contacté récemment', 'contactés récemment')"></span> · <span x-text="summary?.invalid_email || 0"></span> <span x-text="word(summary?.invalid_email, 'adresse invalide', 'adresses invalides')"></span></p></div>
                        ...
                        @else
                            <div class="grid gap-2 sm:grid-cols-2"><button name="action" value="draft" class="rounded-md border border-gray-300 px-3 py-2 text-sm font-semibold text-gray-700">Enregistrer le brouillon</button><button name="action" value="schedule" @click="if (summary && !confirm('Programmer cette campagne pour environ ' + summary.eligible + ' ' + word(summary.eligible, 'destinataire actuellement joignable', 'destinataires actuellement joignables') + ' ? L’audience sera revérifiée au moment de l’envoi.')) $event.preventDefault()" class="rounded-md bg-[#647a0b] px-3 py-2 text-sm font-semibold text-white">Programmer</button></div>
                            <button name="action" value="send_now" class="w-full text-sm font-semibold text-[#647a0b]" @click="if (!summary || !confirm('Envoyer dès que possible à environ ' + summary.eligible + ' ' + word(summary.eligible, 'destinataire actuellement joignable', 'destinataires actuellement joignables') + ' ? L’audience sera revérifiée avant envoi.')) $event.preventDefault()">Envoyer dès que possible</button>
                        @endif
                    </form>
                </section>
            </aside>
