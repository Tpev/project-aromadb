                ...
                <div :class="mobileTab==='content' ? 'block' : 'hidden'" class="space-y-4 lg:block">
                    <fieldset :disabled="locked" class="space-y-4 disabled:opacity-70">
                        ...
                        <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                            ...
                            <div class="mt-3 rounded-md bg-[#f7f9ec] p-3 text-sm text-gray-700" aria-live="polite"><span x-show="estimating">Calcul en cours…</span><template x-if="!estimating && estimate"><p><strong x-text="estimate.eligible"></strong> <span x-text="Number(estimate.eligible) === 1 ? 'destinataire actuellement joignable' : 'destinataires actuellement joignables'"></span> sur <span x-text="estimate.matching"></span> <span x-text="Number(estimate.matching) === 1 ? 'personne correspondante' : 'personnes correspondantes'"></span>.</p></template><span x-show="!estimating && !estimate">Sélectionnez une audience pour obtenir une estimation.</span></div>
                        </section>
                    </fieldset>
