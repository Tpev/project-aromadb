<x-app-layout>
    ...
    <div class="container mt-5">
        <div class="details-container mx-auto p-4">
            <h1 class="details-title">{{ __('Disponibilités Ponctuelles (Dates multiples)') }}</h1>

            <p class="mb-4 text-gray-700">
                {{ __("Sélectionnez une ou plusieurs dates, puis définissez des horaires communs pour ces créneaux ponctuels (ouvertures exceptionnelles).") }}
            </p>

            <form action="{{ route('special-availabilities.store') }}" method="POST">
                @csrf
                ...
                <!-- Dates multiples -->
                <div class="details-box">
                    <label class="details-label" for="dates">{{ __('Dates (une ou plusieurs)') }}</label>
                    <input type="text" id="dates" name="dates" class="form-control" value="{{ old('dates') }}" autocomplete="off">
                    <small class="form-text text-muted">
                        {{ __("Cliquez pour sélectionner plusieurs dates dans le calendrier (mode multi-sélection).") }}
                    </small>
                    @error('dates')
                        <p class="text-red-500">{{ $message }}</p>
                    @enderror
                ...
                <div class="details-box">
                    <div class="form-check">
                        ...
                        <label class="form-check-label" for="applies_to_all">{{ __('Appliquer ces disponibilités à tous les produits') }}</label>
                    </div>
                ...
                <div class="d-flex justify-content-center mt-4">
                    <button type="submit" class="btn-primary">
                        <i class="fas fa-plus mr-2"></i> {{ __('Ajouter les Disponibilités Ponctuelles') }}
                    </button>
