            ...
            <form action="{{ route('availabilities.update', $availability->id) }}" method="POST">
                ...
                <div class="details-box">
                    ...
                    <select id="day_of_week" name="day_of_week" class="form-control" required>
                        ...
                        @foreach(['0' => 'Dimanche', '1' => 'Lundi', '2' => 'Mardi', '3' => 'Mercredi', '4' => 'Jeudi', '5' => 'Vendredi', '6' => 'Samedi'] as $key => $day)
                            <option value="{{ $key }}" {{ old('day_of_week', $availability->day_of_week) === (string)$key ? 'selected' : '' }}>{{ $day }}</option>
                        @endforeach
                    </select>
                    @error('day_of_week')
