...
#remoteVideo {
    ...
    @section('meta_description')
        Join our secure video conference room and enjoy a smooth and professional communication experience.
    @endsection
...
<video id="remoteVideo" autoplay class="absolute top-0 left-0 w-full h-full object-cover z-0"></video>
        ...
        <div id="loadingOverlay" class="absolute inset-0 flex flex-col items-center justify-center bg-black bg-opacity-50 z-20">
            ...
            <p class="text-white text-lg">Waiting for the other participant to join...</p>
        </div>

        <!-- Header -->
        <header class="bg-blue-600 text-white p-4 shadow-md flex justify-between items-center relative z-30">
            <h1 class="text-xl font-semibold">Room: {{ ucfirst($room) }}</h1>
            <span id="connectionStatus" class="text-sm text-yellow-500">Connecting...</span>
        </header>
        ...
        <main class="flex-1 flex flex-col md:flex-row items-end md:items-start justify-end p-4 relative z-30 space-y-4 md:space-y-0 md:space-x-4">
            ...
            <div class="relative w-32 h-24 md:w-40 md:h-32">
                ...
                <span class="absolute bottom-1 left-1 bg-black bg-opacity-50 text-white text-xs px-1 py-0.5 rounded">You</span>
            </div>

            <!-- Control Panel -->
            <div class="flex space-x-4">
                <!-- Toggle Audio Button -->
                <button id="toggleAudio" aria-label="Mute or Unmute Audio" class="bg-blue-500 hover:bg-blue-600 text-white p-4 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400">
                    <i id="audioIcon" class="fas fa-microphone"></i>
                </button>

                <!-- Toggle Video Button -->
                <button id="toggleVideo" aria-label="Mute or Unmute Video" class="bg-blue-500 hover:bg-blue-600 text-white p-4 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400">
                    <i id="videoIcon" class="fas fa-video"></i>
                </button>

                <!-- Full-Screen Toggle Button -->
                <button id="toggleFullScreen" aria-label="Toggle Full-Screen Mode" class="bg-blue-500 hover:bg-blue-600 text-white p-4 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400">
                    <i class="fas fa-expand"></i>
                </button>
<button id="toggleChat" aria-label="Toggle Chat" class="bg-blue-500 hover:bg-blue-600 text-white p-3 rounded-full focus:outline-none focus:ring-2 focus:ring-blue-400 relative">
    <i class="fas fa-comments"></i>
    <span id="chatNotificationBadge" class="notification-badge invisible">0</span> <!-- Initially hidden -->
</button>

                <!-- End Call Button -->
                <button id="endCall" aria-label="End Call" class="bg-red-500 hover:bg-red-600 text-white p-4 rounded-full focus:outline-none focus:ring-2 focus:ring-red-400">
            ...
            <div id="chatPanel" class="absolute top-20 right-4 w-64 bg-white bg-opacity-95 p-4 rounded-lg shadow-lg hidden md:flex flex-col z-40">
                ...
                <input type="text" id="chatInput" placeholder="Type a message..." class="w-full p-2 border rounded" />
            </div>
        </main>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                ...
                function createPeer(initiator, stream) {
                    ...
                    peer.on('connect', () => {
                        ...
                        if (loadingOverlay) {
                            loadingOverlay.classList.add('hidden');
                            console.log('Loading overlay hidden (connect).');
                        }
                    });
                    ...
                    peer.on('close', () => {
                        ...
                        if (loadingOverlay) {
                            loadingOverlay.classList.add('hidden');
                            console.log('Loading overlay hidden (close).');
                        }
                    });
                ...
                function getOffer() {
                    if (offerRetryCount >= MAX_OFFER_RETRIES) {
                        console.error('Max offer retries reached.');
                    ...
                    offerRetryCount++;
                    console.log(`Fetching offer from signaling server. Attempt ${offerRetryCount}`);
                ...
                function checkForAnswer() {
                    if (answerRetryCount >= MAX_ANSWER_RETRIES) {
                        console.error('Max answer retries reached.');
...
toggleChatButton.addEventListener('click', () => {
    ...
    if (isVisible) {
        ...
        toggleChatButton.classList.remove('bg-blue-600');
        toggleChatButton.classList.add('bg-blue-500');
