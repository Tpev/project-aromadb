...
<x-app-layout>
    ...
    <section class="relative overflow-hidden border-b border-[#edf1df]">
        <div class="absolute inset-0 -z-10 bg-gradient-to-b from-[#4E5F3A]/20 via-white to-[#f7f9ef]"></div>
                        ...
                        <a href="{{ route('pro.articles.show', ['therapist' => $therapist->slug, 'articleSlug' => $a->slug]) }}"
                           class="group block bg-white rounded-3xl shadow-sm border border-[#edf1df] overflow-hidden card-hover hover:shadow-md">
                            ...
                            <div class="relative">
                                ...
                                @else
                                    <div class="w-full h-48 bg-gradient-to-br from-[#4E5F3A]/20 via-white to-[#f7f9ef]"></div>
                                @endif

                                <div class="absolute bottom-3 left-3 flex items-center gap-2">
                                    <span class="inline-flex items-center gap-2 rounded-full px-3 py-1 text-xs font-extrabold bg-white/90 border border-[#edf1df] text-[#6B4A3A] backdrop-blur">
                                        <span class="dot bg-[#4E5F3A]"></span>
                                        {{ optional($a->published_at)->format('d/m/Y') }}
                                    </span>

                                    @if($a->reading_time)
                                        <span class="inline-flex items-center rounded-full px-3 py-1 text-xs font-extrabold bg-white/90 border border-[#edf1df] text-[#6B4A3A] backdrop-blur">
                                            {{ $a->reading_time }} min
                                        </span>
                                    @endif
                                </div>
                            ...
                            <div class="p-6">
                                ...
                                <div class="mt-2 text-xl font-black tracking-tight text-gray-900 group-hover:text-[#6B4A3A] transition">
                                    {{ $a->title }}
                                </div>
                                ...
                                <div class="mt-5 flex items-center justify-between">
                                    <div class="text-sm font-semibold text-[#6B4A3A]">
                                        Lire l’article →
                                    </div>
