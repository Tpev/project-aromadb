<x-app-layout>
    ...
    <div class="container-fluid mt-5">
        <div class="details-container mx-auto p-4">
            ...
            <form id="quoteEditForm" action="{{ route('invoices.updateQuote', $quote->id) }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="details-box">
                    ...
                    <div style="display:flex; gap:10px; flex-wrap:wrap;">
                        ...
                        @endphp
                        <select id="global_discount_type" name="global_discount_type" class="form-control" style="max-width:180px;" onchange="recomputeAllTotals()">
                        ...
                        <input id="global_discount_value" type="number" name="global_discount_value" class="form-control" style="max-width:220px;"
                               step="0.01" min="0" value="{{ $gVal }}" oninput="recomputeAllTotals()">
                ...
                <div class="details-box">
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            @if($item->type === 'product')
                                                ...
                                                <select name="items[{{ $i }}][product_id]" class="form-control product-select" onchange="onProductChange(this)">
                                            ...
                                            @else
                                                <input type="hidden" name="items[{{ $i }}][product_id]" value="">
                                                <input type="hidden" name="items[{{ $i }}][inventory_item_id]" value="">
                                                <input type="text" class="form-control custom-name"
                                        ...
                                        <td>
                                            @if($item->type === 'custom')
                                                <input type="text" class="form-control custom-details"
                                            ...
                                            @else
                                                <input type="text" name="items[{{ $i }}][description]" class="form-control"
                                                       value="{{ $rawDesc }}" oninput="recomputeAllTotals()">
                                            @endif
                                        </td>

                                        <td>
                                            <input type="number" name="items[{{ $i }}][quantity]" class="form-control quantity-input"
                                                   min="0.01" step="0.01"
                                        ...
                                        <td>
                                            <input type="number" name="items[{{ $i }}][unit_price]" class="form-control unit-price-input"
                                                   step="0.01"
                                        ...
                                        <td>
                                            <input type="number" name="items[{{ $i }}][tax_rate]" class="form-control tax-rate-input"
                                                   step="0.01"
                                        ...
                                        <td>
                                            <select name="items[{{ $i }}][line_discount_type]" class="form-control line-discount-type" onchange="recomputeAllTotals()">
                                        ...
                                        <td>
                                            <input type="number" name="items[{{ $i }}][line_discount_value]" class="form-control line-discount-value"
                                                   step="0.01" min="0" value="{{ $ldVal }}" oninput="recomputeAllTotals()">
                                        </td>

                                        <td>
                                            <input type="number" class="form-control line-discount-amt readonly-field" step="0.01" readonly>
                                        </td>

                                        <td>
                                            <input type="number" name="items[{{ $i }}][total_price]" class="form-control total-ht readonly-field" step="0.01"
                                                   value="{{ old("items.$i.total_price", $item->total_price) }}" readonly>
                                        </td>

                                        <td>
                                            <input type="number" name="items[{{ $i }}][tax_amount]" class="form-control tax-amt readonly-field" step="0.01"
                                                   value="{{ old("items.$i.tax_amount", $item->tax_amount) }}" readonly>
                                        </td>

                                        <td>
                                            <input type="number" name="items[{{ $i }}][total_price_with_tax]" class="form-control total-ttc readonly-field" step="0.01"
                                                   value="{{ old("items.$i.total_price_with_tax", $item->total_price_with_tax) }}" readonly>
                                        </td>
                    ...
                    <div class="flex gap-2 mt-2 flex-wrap">
                        <button type="button" class="btn-primary" onclick="addProductItem()">{{ __('Ajouter une prestation') }}</button>
                        <button type="button" class="btn-primary" onclick="openInventoryModal()">{{ __('Ajouter depuis l\'inventaire') }}</button>
    ...
    <div id="inventoryModal" class="fixed inset-0 bg-black bg-opacity-50 hidden items-center justify-center z-50">
        <div class="bg-white rounded-lg p-6 w-full max-w-xl shadow-lg">
            ...
            <div class="flex justify-end gap-3">
                <button type="button" class="btn-secondary" onclick="closeInventoryModal()">{{ __('Annuler') }}</button>
                <button type="button" class="btn-primary" onclick="addInventoryItemFromModal()">{{ __('Ajouter') }}</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        function addProductItem() {
            ...
            row.innerHTML = `
                <td>Prest.<input type="hidden" name="items[${idx}][type]" value="product"></td>
                <td>
        ...
        function addCustomItem() {
            ...
            row.innerHTML = `
                <td>Libre<input type="hidden" name="items[${idx}][type]" value="custom"></td>
                <td>
        ...
        function addInventoryItemFromModal() {
            ...
            row.innerHTML = `
                <td>Inv.<input type="hidden" name="items[${idx}][type]" value="inventory"></td>
                <td>
                    <input type="hidden" name="items[${idx}][product_id]" value="">
                ...
                <td><input type="text" name="items[${idx}][description]" class="form-control" oninput="recomputeAllTotals()"></td>
                <td><input type="number" name="items[${idx}][quantity]" class="form-control quantity-input" value="${qty}" min="0.01" step="0.01" oninput="recomputeAllTotals()"></td>
                <td><input type="number" name="items[${idx}][unit_price]" class="form-control unit-price-input" value="${unitHt.toFixed(2)}" step="0.01" oninput="recomputeAllTotals()"></td>
                <td><input type="number" name="items[${idx}][tax_rate]" class="form-control tax-rate-input" value="${taxRate.toFixed(2)}" step="0.01" readonly></td>
