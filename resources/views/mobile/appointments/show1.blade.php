...
@php
    ...
    $whenBadge = $isPast
        ...
        : 'bg-[#6B4A3A]/10 text-[#6B4A3A] border-[#d7dfaa]';
@endphp
    ...
    <div class="px-4 pt-4 pb-24 space-y-4">
            ...
            <div class="flex items-start justify-between gap-3">
                <div>
                    ...
                    <p class="mt-1 flex items-center gap-2 text-sm font-semibold text-gray-900">
                        <i class="fas fa-clock text-[11px] text-[#6B4A3A]"></i>
                        {{ $date->format('H:i') }}
        ...
        <div class="rounded-2xl border border-[#D8CFBF] bg-white p-4 shadow-sm">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 rounded-full bg-[#6B4A3A]/10 flex items-center justify-center text-[#6B4A3A]">
            ...
            <div class="mt-3">
                <a href="{{ route('client_profiles.show', $client->id) }}"
                   class="inline-flex items-center gap-1.5 text-[11px] text-[#6B4A3A] font-medium">
        ...
        <div class="rounded-2xl border border-[#D8CFBF] bg-white p-4 shadow-sm">
            ...
            <p class="text-sm font-semibold text-gray-900 flex items-center gap-2">
                <span class="inline-flex items-center justify-center w-7 h-7 rounded-full bg-[#6B4A3A]/10 text-[#6B4A3A]">
            ...
            <div class="mt-3 grid grid-cols-2 gap-3 text-xs">
                ...
                <div class="flex flex-col gap-1">
                    ...
                    <span class="text-gray-800 flex items-center gap-1.5">
                        <i class="fas {{ $modeIcon }} text-[11px] text-[#6B4A3A]"></i>
                        {{ $mode }}
                    </span>
                </div>
            </div>
            ...
            <div class="rounded-2xl border border-[#D8CFBF] bg-white p-4 shadow-sm space-y-3">
                ...
                @if($meetingLink)
                    <div class="text-xs">
                        ...
                        <a href="{{ $meetingLink }}"
                           class="inline-flex items-center justify-between w-full px-3 py-2 rounded-xl bg-[#6B4A3A] text-white text-[11px] font-medium active:scale-[0.99] transition">
                    ...
                    <div class="text-xs">
                        ...
                        <div class="flex flex-col gap-2">
                            <button
                                ...
                                "
                                class="inline-flex items-center justify-between w-full px-3 py-2 rounded-xl bg-white border border-[#D8CFBF] text-[11px] text-gray-700 active:scale-[0.99] transition">
                                <span class="flex items-center gap-2">
                                    <i class="fas fa-link text-[11px] text-[#6B4A3A]"></i>
                                    Copier le lien pour le client
                                </span>
                                <span x-show="!copied" class="text-[10px] text-gray-400">Copier</span>
                                <span x-show="copied" x-cloak class="text-[10px] text-[#6B4A3A] font-semibold">Copié ✓</span>
                            </button>
                        </div>
                    </div>
                @endif
        ...
        <div class="space-y-3">
            ...
            <div class="grid grid-cols-2 gap-2">
                <a href="{{ route('appointments.edit', $appointment->id) }}"
                   class="inline-flex items-center justify-center px-3 py-2 rounded-xl bg-white border border-[#D8CFBF] text-[12px] font-medium text-gray-800 active:scale-[0.99] transition">
            ...
            <div class="rounded-2xl border border-[#D8CFBF] bg-white p-2 shadow-sm text-[12px]">
                {{-- Mark as completed --}}
                ...
                @can('update', $appointment)
                    @if($appointment->product?->requires_emargement && !$appointment->emargement_sent)
                        <form action="{{ route('emargement.send', $appointment->id) }}"
                            ...
                            @csrf
                            <button type="submit"
                                    class="w-full flex items-center justify-between px-3 py-2 rounded-xl text-left text-[#6B4A3A]">
                                <span class="flex items-center gap-2">
