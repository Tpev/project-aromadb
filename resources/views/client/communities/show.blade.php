    ...
    <div class="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8 space-y-6">
        <section class="overflow-hidden rounded-[2rem] border border-[#dfe7c7] bg-[radial-gradient(circle_at_top_left,_rgba(224,235,198,0.72),_rgba(255,255,255,0.95)_42%,_rgba(244,240,231,0.95)_100%)] p-6 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.22em] text-[#854f38]">Communaute privee</p>
                    <h1 class="mt-2 text-3xl font-semibold tracking-tight text-gray-900">{{ $community->name }}</h1>
                    <p class="mt-3 max-w-3xl text-sm leading-7 text-gray-600">Animee par votre praticien {{ $community->user->company_name ?? $community->user->name }}, cette communaute vous permet de suivre les annonces importantes et de participer a des echanges plus personnels qu'un espace public.</p>
                </div>
                <a href="{{ route('client.communities.index') }}" class="inline-flex items-center rounded-full bg-white/90 px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-[#e7eadf] hover:text-[#647a0b]">Toutes mes communautes</a>
            </div>
        </section>

        <div class="grid gap-6 xl:grid-cols-[290px_minmax(0,1fr)_280px]">
            <aside class="overflow-hidden rounded-[2rem] border border-[#eceee5] bg-white shadow-sm">
                <div class="border-b border-[#f0f1ea] px-5 py-5">
                    ...
                    <p class="mt-3 text-sm leading-7 text-gray-600">Passez facilement d'un espace d'annonce a un espace de discussion sans perdre le fil.</p>
                </div>
                ...
                @if($selectedChannel)
                                ...
                                <div>
                                    <div class="flex flex-wrap items-center gap-3">
                                        ...
                                        <span class="rounded-full px-3 py-1 text-xs font-semibold {{ $selectedChannel->channel_type === 'annonces' ? 'bg-amber-100 text-amber-700' : 'bg-lime-100 text-lime-700' }}">
                                            {{ $selectedChannel->channel_type === 'annonces' ? 'Lecture prioritaire' : 'Echange ouvert' }}
                                        </span>
                                    </div>
                                    <p class="mt-2 text-sm leading-7 text-gray-500">{{ $selectedChannel->description ?: ($selectedChannel->channel_type === 'annonces' ? 'Retrouvez ici les messages publies par votre praticien.' : 'Echangez librement avec les autres membres de cette communaute.') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            ...
                            @empty
                                <div class="rounded-[1.8rem] border border-dashed border-[#d7dccb] bg-white/85 px-8 py-14 text-center shadow-sm">
                                    ...
                                    <p class="mt-2 text-sm leading-7 text-gray-500">Le salon est pret. Vous pouvez attendre la prochaine annonce du praticien ou lancer le premier echange du groupe.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
                    ...
                    @elseif($community->is_archived)
                        <div class="border-t border-[#f0f1ea] bg-gray-50 px-6 py-4 text-sm leading-6 text-gray-600">
                            Cette communaute est archivee. Vous pouvez relire les echanges mais vous ne pouvez plus envoyer de message.
                        </div>
                    @else
                            ...
                            <form method="POST" action="{{ route('client.communities.messages.store', $community) }}" class="rounded-[1.8rem] border border-[#eceee5] bg-[linear-gradient(180deg,_rgba(250,251,247,1),_rgba(255,255,255,1))] p-4 shadow-inner shadow-[#f3f4ef]">
                                ...
                                <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                                    <div>
                                        ...
                                        <p class="mt-1 text-xs text-gray-500">Partagez un retour, une question ou une reaction utile au groupe.</p>
                                    </div>
                                ...
                                <textarea name="content" rows="4" class="mt-4 w-full rounded-[1.4rem] border-gray-300 bg-white focus:border-[#647a0b] focus:ring-[#647a0b]" placeholder="Partagez votre retour, posez une question ou repondez au groupe."></textarea>
            ...
            <aside class="space-y-5">
                <section class="overflow-hidden rounded-[2rem] border border-[#eceee5] bg-white shadow-sm">
                    <div class="border-b border-[#f0f1ea] px-5 py-5">
                        ...
                        <p class="mt-2 text-sm leading-6 text-gray-500">Vous etes dans une communaute privee. Les echanges restent visibles uniquement par les membres admis.</p>
                    </div>
                    <div class="space-y-3 px-5 py-5 text-sm leading-7 text-gray-600">
                        <div class="rounded-[1.4rem] bg-[#fafbf7] px-4 py-3 ring-1 ring-[#eef1e5]">
                            ...
                            <p class="mt-2 font-semibold text-gray-900">{{ $community->user->company_name ?? $community->user->name }}</p>
                        </div>
                        <div class="rounded-[1.4rem] bg-[#fafbf7] px-4 py-3 ring-1 ring-[#eef1e5]">
                            <p class="text-xs uppercase tracking-[0.16em] text-gray-500">Statut</p>
                            <p class="mt-2 font-semibold text-gray-900">{{ $community->is_archived ? 'Communaute archivee' : 'Communaute active' }}</p>
                        </div>
                    </div>
                </section>
            </aside>
