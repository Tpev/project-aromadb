    ...
    <div class="mx-auto max-w-6xl space-y-8 p-4 sm:p-6 lg:p-8">
        <section class="overflow-hidden rounded-[2rem] border border-[#dfe7c7] bg-[radial-gradient(circle_at_top_left,_rgba(224,235,198,0.72),_rgba(255,255,255,0.95)_45%,_rgba(244,240,231,0.95)_100%)] p-8 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                <div>
                    <p class="text-xs font-semibold uppercase tracking-[0.24em] text-[#5F7048]">Espace privé</p>
                ...
                <a href="{{ route('client.home') }}" class="inline-flex items-center rounded-full border border-[#d7ddc8] bg-white/90 px-4 py-2.5 text-sm font-semibold text-gray-700 shadow-sm hover:border-[#6B4A3A] hover:text-[#6B4A3A]">
                    Retour à l’accueil
                </a>
            </div>
        </section>
                    ...
                    @foreach($pendingInvites as $membership)
                        <article class="rounded-[1.8rem] border border-amber-200/70 bg-white/90 p-5 shadow-sm">
                            ...
                            <form method="POST" action="{{ route('client.communities.accept', $membership->group) }}" class="mt-5">
                                @csrf
                                <button type="submit" class="inline-flex items-center rounded-full bg-[#6B4A3A] px-4 py-2.5 text-sm font-semibold text-white shadow-sm shadow-lime-900/10 transition hover:-translate-y-0.5 hover:bg-[#55670a]">
        ...
        <section class="rounded-[2rem] border border-[#eceee5] bg-white p-6 shadow-sm">
            <div class="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                ...
                <span class="rounded-full bg-[#f7faef] px-4 py-1.5 text-sm font-semibold text-[#6B4A3A]">{{ $communities->count() }} active(s)</span>
            </div>
            <div class="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
                @forelse($communities as $membership)
                    <article class="group overflow-hidden rounded-[1.8rem] border border-[#eceee5] bg-[linear-gradient(180deg,_rgba(249,250,246,1),_rgba(255,255,255,1))] p-5 shadow-sm transition hover:-translate-y-1 hover:shadow-lg hover:shadow-lime-900/5">
                        ...
                        <a href="{{ route('client.communities.show', $membership->group) }}" class="mt-5 inline-flex items-center rounded-full bg-[#6B4A3A] px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-[#55670a]">
                ...
                @empty
                    <div class="md:col-span-2 xl:col-span-3 rounded-[1.8rem] border border-dashed border-[#d7dccb] bg-[#fafbf7] px-8 py-14 text-center">
                        <div class="mx-auto flex h-14 w-14 items-center justify-center rounded-3xl bg-white text-xl font-semibold text-[#6B4A3A] shadow-sm">#</div>
