        ...
        <div class="am-card p-5">
            ...
            <form action="{{ route('client_profiles.packs.assign', $clientProfile->id) }}" method="POST" class="mt-4 space-y-3">
                ...
                {{-- =======================
                     Onglet : Rendez-vous
                     ======================= --}}
                <div x-show="tab === 'Rendez-vous'" x-cloak>
                    <div class="row mt-4">
                        <div class="col-md-12">
                                                        ...
                                                        @if($canSend || $em)
                                                            ...
                                                            @if($canSend)
                                                                <form action="{{ route('emargement.send', $appointment->id) }}"
                                                                    ...
                                                                    <button type="submit"
                                                                            class="btn-primary"
                                                                            title="Envoyer la feuille d’émargement">
                                                                        Envoyer signature
                                                                    </button>
                                                                </form>
                                                            @endif
                ...
                {{-- =======================
                     Onglet : Notes de séance
                     ======================= --}}
                <div x-show="tab === 'Notes de séance'" x-cloak>
                    <div class="row mt-4">
                        <div class="col-md-12">
                            <h2 class="details-title">{{ __('Notes de Séance de ce client') }}</h2>
                            <a href="{{ route('session_notes.create', $clientProfile->id) }}"
                               class="btn btn-primary mb-3">
                                        ...
                                        <tbody>
                                            @foreach($sessionNotes as $sessionNote)
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="{{ route('session_notes.show', $sessionNote->id) }}"
                                                           class="btn btn-primary">
                                                            Voir
                                                        </a>
                                                        <a href="{{ route('session_notes.edit', $sessionNote->id) }}"
                                                           class="btn btn-secondary">
                                                            Modifier
                                                        </a>
                                                    </td>
                ...
                {{-- =======================
                     ...
                     ======================= --}}
                <div x-show="tab === 'Questionnaires'" x-cloak>
                    <div class="row mt-4">
                        <div class="col-md-12">
                ...
                {{-- =======================
                     Onglet : Conseils envoyés
                     ======================= --}}
                <div x-show="tab === 'Conseils envoyés'" x-cloak>
                    <div class="row mt-4">
                        <div class="col-md-12">
                            <h2 class="details-title">{{ __('Conseils Envoyés à ce client') }}</h2>
                            <a href="{{ route('client_profiles.conseils.sendform', $clientProfile->id) }}"
                               class="btn btn-primary mb-3">
                ...
                {{-- =======================
                     Onglet : Mesures
                     ======================= --}}
                <div x-show="tab === 'Mesures'" x-cloak>
                    <div class="row mt-4">
                        <div class="col-md-12">
                            <h2 class="details-title">{{ __('Mesures du client') }}</h2>

                            <a href="{{ route('client_profiles.metrics.create', $clientProfile->id) }}"
                               class="btn btn-primary mb-3">
                ...
                {{-- =======================
                     Onglet : Messagerie
                     ======================= --}}
                <div x-show="tab === 'Messagerie'" x-cloak>
                ...
                {{-- =======================
                     Onglet : Fichiers & Documents
                     ======================= --}}
                <div x-show="tab === 'Fichiers & Documents'" x-cloak>
                    <!-- ==========================
                         Fichiers classiques
                         ========================== -->
                    <div class="row mt-4">
                    ...
                    <section class="bg-white p-6 rounded-2xl shadow mt-6" id="documents-signing">
                        ...
                        <div class="text-sm text-gray-600 mb-4">
                            ...
                            @else
                                <strong class="text-red-600">
                                    Aucune — ajoutez un email au profil pour activer l’envoi
                                </strong>
                            @endif
                        </div>
                        ...
                        <form action="{{ route('documents.store', $clientProfile->id) }}"
                            ...
                            <div class="md:col-span-4">
                                <label for="appointment_id" class="block font-semibold mb-1">
                                ...
                                <select id="appointment_id"
                                    ...
                                    @foreach($appointments as $appt)
                                        <option value="{{ $appt->id }}">
                                            ...
                                            — {{ optional($appt->product)->name ?? 'Prestation' }}
                                        </option>
                                    @endforeach
                                </select>
                            </div>
                        ...
                        @if(session('warning'))
                            <div class="mt-3 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-semibold text-amber-800">
                                {{ session('warning') }}
                            </div>
                        @endif
                        ...
                        @php
                            ...
                            $newId = session('new_document_id');
                        @endphp

                        @if($documents->isEmpty())
                            <p class="text-muted">
                                Aucun document n’a encore été importé pour ce client.
                            </p>
                        @else
                                            ...
                                            <tr data-doc-id="{{ $doc->id }}"
                                                ...
                                                <td title="{{ $doc->original_name }}">
                                                    <div class="flex items-center gap-2">
                                                        ...
                                                        <a href="{{ asset('storage/'.$doc->storage_path) }}"
                                                           target="_blank">
                                                ...
                                                <td>
                                                    ...
                                                    @if($signedCnt > 0)
                                                        <span class="text-muted">
                                                            — dernier :
                                                            {{ optional($events->sortByDesc('signed_at')->first()?->signed_at)
                                                                ?->format('d/m/Y H:i') ?? '—' }}
                                                        </span>
                                                    @endif
                                                </td>
                                                <td>
                                                    @if($expires)
                                                        ...
                                                        @else
                                                            <span class="text-muted">
                                                                Expire le {{ $expires->format('d/m/Y H:i') }}
                                                            </span>
                                                        @endif
                                                    @else
                                                ...
                                                <td style="min-width:420px">
                                                    ...
                                                    {{-- Envoyer au client --}}
                                                    <form action="{{ route('documents.send', $doc->id) }}"
                                                          method="POST"
                                                          style="display:inline-block;">
                                                        @csrf
                                                        <button type="submit"
                                                    ...
                                                    {{-- Renvoyer le lien --}}
                                                    @if($canResend)
                                                        <form action="{{ route('documents.resend', $signing->id) }}"
                                                              method="POST"
                                                              style="display:inline-block;">
                                                            @csrf
                                                            <button type="submit" class="btn-secondary">
                                                    ...
                                                    @if($publicUrl)
                                                        <button type="button"
                                                                class="btn-secondary copy-link-btn"
                                                                data-url="{{ $publicUrl }}">
                                                            Copier le lien
                                                        </button>
                                                    @endif
                                                    ...
                                                    @if($canDownload)
                                                        <a href="{{ route('documents.download.final', $doc->id) }}"
                                                           class="btn-secondary">
                                                            Télécharger le PDF final
                                                        </a>
                                                    @endif
