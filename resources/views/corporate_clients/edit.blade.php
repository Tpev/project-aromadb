...
<x-app-layout>
    ...
    <div class="container mt-5">
        <div class="details-container mx-auto p-4">
            ...
            <h1 class="details-title">{{ $company->name }}</h1>

            <form action="{{ route('corporate-clients.update', $company) }}" method="POST">
                @csrf
                @method('PUT')

                @include('corporate_clients._form', ['company' => $company])

                <button type="submit" class="btn-primary mt-4">
                    {{ __('Mettre à jour') }}
                </button>
                <a href="{{ route('corporate-clients.show', $company) }}" class="btn-secondary mt-4">
                    {{ __('Annuler') }}
                </a>
            </form>
        </div>
