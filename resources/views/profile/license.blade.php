            ...
            @if(auth()->user()->isTherapist())
                ...
                <div class="section">
                    ...
                    <div class="section-content text-center">
                        @if(auth()->user()->license_product)
                            <p><strong>{{ __('Licence Actuelle :') }}</strong> {{ auth()->user()->license_product }}</p>
                            
                            @if(auth()->user()->license_product == 'essai')
                                <a href="{{ route('license-tiers.pricing') }}" class="btn-primary mt-4">
                                    {{ __('Passez à la licence complète') }}
                                </a>
                            @endif
