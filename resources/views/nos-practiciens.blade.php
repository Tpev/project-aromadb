...
Découvrez notre annuaire thérapeute dédié aux médecines douces. Sophrologie, naturopathie, ostéopathie : trouvez le professionnel certifié idéal pour votre bien-être
  ...
  <section class="relative bg-gradient-to-r from-green-50 to-white overflow-hidden h-screen"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full flex flex-col items-center justify-center relative z-10">
      <div class="text-center"
        ...
        <h1 class="text-4xl sm:text-5xl font-bold mb-4 text-[#6B4A3A]">
          Trouvez votre praticien en médecine douce
        </h1>
        <p class="max-w-3xl mx-auto text-lg sm:text-xl mb-8 text-[#6B4A3A]">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8"
        ...
        <div class="bg-white shadow-lg rounded-xl p-6 flex items-center space-x-4 hover:shadow-2xl transition-shadow duration-300">
          <div class="text-5xl text-[#6B4A3A]">
            <i class="fas fa-check-circle"></i>
          </div>
          <div>
            <h3 class="text-xl font-semibold mb-2 text-[#6B4A3A]">
              Praticiens Vérifiés
            </h3>
            <p class="text-[#6B4A3A]">
  ...
  <section class="py-16">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 grid grid-cols-1 lg:grid-cols-2 gap-10 items-center">
      <div class="relative" x-data="{ reveal: false }" x-init="setTimeout(() => reveal = true, 200)">
        ...
        <div class="absolute bottom-0 right-0 bg-[#5F7048] text-white px-4 py-2 rounded-tl-lg shadow-lg">
          100% praticiens
        </div>
      </div>
      <div class="space-y-6">
        <h2 class="text-3xl font-bold text-[#6B4A3A]">
          Pourquoi choisir nos praticiens ?
        </h2>
        <p class="text-xl text-[#6B4A3A]">
        ...
        <a href="#" class="inline-block mt-4 bg-[#6B4A3A] text-white px-6 py-3 rounded-full shadow hover:bg-[#4E5F3A] transition">
          En savoir plus
        </a>
      </div>
    </div>
  ...
  <section class="py-12 bg-[#f8f8f8]"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="flex justify-between items-center mb-8">
        <h2 class="text-3xl font-bold text-[#4E5F3A]">
          Actualités & Conseils Bien-Être
        </h2>
        <a href="{{ route('blog.index') }}" class="text-[#5F7048] hover:text-[#4E5F3A] font-semibold flex items-center">
        ...
        @forelse($blogPosts as $post)
          <div class="bg-white shadow-xl rounded-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300">
            ...
            <div class="p-6 space-y-3">
              @if($post->Tags)
                <span class="inline-block bg-[#6B4A3A] text-white text-xs px-3 py-1 rounded-full">
                  {{ $post->Tags }}
                </span>
              @endif
              <h3 class="text-2xl font-semibold text-[#5F7048]">
                {{ $post->Title }}
              </h3>
              <p class="text-xl text-[#6B4A3A]">
              ...
              <a href="{{ route('blog.show', $post->slug) }}"
                 class="text-[#5F7048] font-medium hover:underline">
                Lire l’article
              </a>
            </div>
          </div>
        @empty
          <p class="text-center text-xl text-[#6B4A3A]">Aucun article disponible.</p>
        @endforelse
      </div>
    </div>
  </section>
  ...
  <section class="py-16 bg-white">
    <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
      <h2 class="text-3xl font-bold text-center mb-8 text-[#6B4A3A]">
        Ils nous font confiance
      </h2>
      <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
        <!-- Témoignage 1 -->
        <div class="bg-gray-100 p-6 rounded-lg shadow-md">
          <p class="italic text-xl mb-4 text-[#6B4A3A]">
          ...
          <div class="flex items-center">
            ...
            <div>
              <p class="font-semibold text-[#6B4A3A]">Sophie L.</p>
              <p class="text-sm text-[#6B4A3A]"></p>
            </div>
          </div>
        </div>
        <!-- Témoignage 2 -->
        <div class="bg-gray-100 p-6 rounded-lg shadow-md">
          ...
          <div class="flex items-center">
            <img src="https://randomuser.me/api/portraits/men/46.jpg"
                 ...
                 class="w-12 h-12 rounded-full mr-4">
            <div>
              <p class="font-semibold text-[#6B4A3A]">Marc D.</p>
              <p class="text-sm text-[#6B4A3A]"></p>
            </div>
          </div>
        </div>
        ...
        <div class="bg-gray-100 p-6 rounded-lg shadow-md">
          ...
          <div class="flex items-center">
            <img src="https://randomuser.me/api/portraits/women/65.jpg"
                 ...
                 class="w-12 h-12 rounded-full mr-4">
            <div>
              <p class="font-semibold text-[#6B4A3A]">Isabelle R.</p>
              <p class="text-sm text-[#6B4A3A]"></p>
            </div>
          </div>
        </div>
    ...
    <div class="max-w-xl mx-auto text-center space-y-6">
      ...
      <form action="#" method="POST" class="flex flex-col sm:flex-row justify-center items-center gap-4 mt-6">
        <input type="email"
               ...
               class="border border-gray-300 rounded-full px-4 py-2 focus:ring-[#6B4A3A] focus:border-[#6B4A3A] w-full sm:w-auto"
               required>
      ...
      <div class="space-y-4">
        <div x-data="{ open: false }" class="border border-gray-200 rounded-lg p-4">
          <button @click="open = !open" class="w-full text-left flex justify-between items-center focus:outline-none">
            <span class="font-semibold text-lg text-[#6B4A3A]">
              Comment se déroule la vérification des praticiens ?
            </span>
            <svg x-show="!open" xmlns="http://www.w3.org/2000/svg"
                 class="h-6 w-6 text-[#6B4A3A]"
          ...
          <div x-show="open" x-transition class="mt-4 text-[#6B4A3A]">
