...
class SuperPdpController extends Controller
{
    public function connect(Request $request, SuperPdpOAuthService $oauthService)
    {
        ...
        if (! $oauthService->isConfigured()) {
            return back()->with('error', 'La connexion SUPER PDP n\'est pas encore configurée côté Olithea.');
        }
    ...
    public function callback(Request $request, SuperPdpOAuthService $oauthService)
    {
        ...
        try {
            ...
            return redirect()
                ->route('profile.editCompanyInfo')
                ->with('error', 'Impossible de finaliser la connexion SUPER PDP. Vérifiez l’application et réessayez.');
        }

        return redirect()
            ->route('profile.editCompanyInfo')
            ->with('success', 'SUPER PDP est connecté. Vous pouvez maintenant utiliser le service.');
    }
    ...
    public function disconnect(Request $request, SuperPdpOAuthService $oauthService)
    {
        ...
        $connection = SuperPdpConnection::query()
            ...
            ->where('environment', $oauthService->environment())
            ->first();
        ...
        return back()->with('success', 'SUPER PDP a été déconnecté.');
    }
}
